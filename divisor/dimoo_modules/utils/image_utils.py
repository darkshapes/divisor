...
def decode_vq_to_image(vq_codes: torch.LongTensor, save_path: str, vae_ckpt: str, image_height: int, image_width: int, vqvae: VQModel | None = None) -> Image.Image:
    """
    ...
    if vq_codes.shape[1] != latent_height * latent_width:
        raise ValueError(f"VQ codes length mismatch: {vq_codes.shape[1]} != {latent_height * latent_width} for image size ({image_height},{image_width}) with scale {scale}")
