...
def activation_checkpoint_function(cfg: ModelConfig):
    preserve_rng_state = (cfg.attention_dropout == 0.0) and (cfg.embedding_dropout == 0.0) and (cfg.residual_dropout == 0.0)
...
class LayerNormBase(nn.Module):
    ...
    def _cast_if_autocast_enabled(self, tensor: torch.Tensor, dtype: Optional[torch.dtype] = None) -> torch.Tensor:
        ...
        if (tensor.device.type == "cuda" or tensor.device.type == "mps") and torch.is_autocast_enabled():
...
class LLaDABlock(nn.Module):
    ...
    def _cast_attn_bias(cls, bias: torch.Tensor, input_dtype: torch.dtype) -> torch.Tensor:
        ...
        # `is_autocast_cpu_enabled()` for CPU autocast.
        # See https://github.com/pytorch/pytorch/issues/110966.
        if (bias.device.type == "cuda" or bias.device.type == "mps") and torch.is_autocast_enabled():
...
class LLaDASequentialBlock(LLaDABlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.att_proj = nn.Linear(config.d_model, sum(self.fused_dims), bias=config.include_bias | config.include_qkv_bias, device=config.init_device)
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.q_proj = nn.Linear(config.d_model, q_proj_out_dim, bias=config.include_bias | config.include_qkv_bias, device=config.init_device)
        self.k_proj = nn.Linear(config.d_model, k_proj_out_dim, bias=config.include_bias | config.include_qkv_bias, device=config.init_device)
        self.v_proj = nn.Linear(config.d_model, v_proj_out_dim, bias=config.include_bias | config.include_qkv_bias, device=config.init_device)

        # Feed-forward input projection.
        self.ff_proj = nn.Linear(config.d_model, self.hidden_size, bias=config.include_bias, device=config.init_device)
        # new add
        self.up_proj = nn.Linear(config.d_model, self.hidden_size, bias=config.include_bias, device=config.init_device)
...
class LLaDAModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if self.config.block_group_size > 1:
            block_groups = [LLaDABlockGroup(config, i, blocks[i : i + config.block_group_size]) for i in range(0, config.n_layers, config.block_group_size)]
    ...
    def forward(
        ...
        if (
            ...
            if attention_bias is None and self.config.alibi:
                attention_bias = get_causal_attention_bias(self.__cache, past_length + seq_len, x.device) + self.get_alibi_attention_bias(past_length + seq_len, x.device)
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    x, _ = block(x, attention_bias=attention_bias, layer_past=layer_past, to_compute_mask=to_compute_mask, use_cache=use_cache, cat=cat)
        else:
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, _ = block_group(x, attention_bias=attention_bias, layers_past=layers_past, to_compute_mask=to_compute_mask, use_cache=use_cache, cat=cat)
                # if attn_key_values is not None:
                #     assert cache is not None
