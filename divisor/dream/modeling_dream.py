...
from divisor.dream.configuration_dream import DreamConfig
from divisor.dream.generation_utils import DreamGenerationMixin, DreamGenerationConfig

if is_flash_attn_2_available():
    from transformers.modeling_flash_attention_utils import _flash_attention_forward

from divisor.trado.modeling_sdar import compute_default_rope_parameters
...
class DreamRotaryEmbedding(nn.Module):
    def __init__(
        ...
        if config is None:
            logger.warning_once(
                "`DreamRotaryEmbedding` can now be fully parameterized by passing the model config through the `config` argument. All other arguments will be removed in v4.46"
            )
            self.rope_kwargs = {
                "rope_type": rope_type,
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def __init__(self, config: DreamConfig):
        ...
        self.layers = nn.ModuleList([DreamDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
