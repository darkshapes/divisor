...
from divisor.fudoki_modules.flow_matching.utils.model_wrapper import ModelWrapper
# type: ignore
...
def top_k_logits(logits, top_k=None):
    top_k = min(top_k, logits.size(-1))  # Safety check #type: ignore
...
class CFGScaledModel(ModelWrapper):
    ...
    def forward(self, x: torch.Tensor, cfg_scale: float, datainfo, uncond_id=100015):
        with torch.no_grad():
