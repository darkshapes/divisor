...
from divisor.fudoki_modules.flow_matching.solver.solver import Solver
from divisor.fudoki_modules.flow_matching.utils import ModelWrapper
from divisor.fudoki_modules.flow_matching.utils.manifolds import geodesic, Manifold
    ...
    def sample(
        ...
        **model_extras,
        ...
        for t0, t1 in zip(t0s, t_discretization[1:]):
            ...
            if return_intermediates:
                while i_ret < len(time_grid) and t0 <= time_grid[i_ret] and time_grid[i_ret] <= t1:
                    xts.append(interp(self.manifold, xt, xt_next, t0, t1, time_grid[i_ret]))
                    i_ret += 1
            xt = xt_next
...
def interp(manifold, xt, xt_next, t, t_next, t_ret):
    return geodesic(manifold, xt, xt_next)((t_ret - t) / (t_next - t).reshape(1)).reshape_as(xt)
...
def _euler_step(
    ...
    """
    velocity_fn = lambda x, t: (manifold.proju(x, velocity_model(x, t)) if proju else velocity_model(x, t))
