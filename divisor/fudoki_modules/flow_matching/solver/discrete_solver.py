...
from divisor.fudoki_modules.flow_matching.path import MixtureDiscreteProbPath, MixtureDiscreteSoftmaxProbPath

from divisor.fudoki_modules.flow_matching.solver.solver import Solver
from divisor.fudoki_modules.flow_matching.utils import categorical, ModelWrapper
from divisor.fudoki_modules.flow_matching.solver.utils import get_nearest_times
            ...
            for i in range(n_steps):
                ...
                if i == n_steps - 1:
                    ...
                    if p_1t_txt is None:
                        ...
                        distance_x_t_2_x_1 = (
                            torch.sum(emb_x_t_flattened**2, dim=1, keepdim=True)
                            + torch.sum(emb_x_1_flattened**2, dim=1, keepdim=True)
                            - 2 * torch.einsum("bd,bd->b", emb_x_t_flattened, emb_x_1_flattened).unsqueeze(1)
                        ) ** 2
