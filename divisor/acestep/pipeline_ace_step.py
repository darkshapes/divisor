...
class ACEStepPipeline:
    ...
    def get_checkpoint_path(self, checkpoint_dir, repo):
        ...
        if checkpoint_dir is not None:
            ...
            if all_dirs_exist:
                nfo(f"Load models from: {checkpoint_dir}")
                checkpoint_dir_models = checkpoint_dir

        if checkpoint_dir_models is None:
            if checkpoint_dir is None:
                nfo(f"Download models from Hugging Face: {repo}")
                checkpoint_dir_models = snapshot_download(repo)
            else:
                nfo(f"Download models from Hugging Face: {repo}, cache to: {checkpoint_dir}")
    ...
    def flowedit_diffusion_process(
        ...
        nfo("flowedit start from {} to {}".format(n_min, n_max))
    ...
    def add_latents_noise(
        ...
        nfo(f"{scheduler.sigma_min=} {scheduler.sigma_max=} {timesteps=} {num_inference_steps=}")
    ...
    def text2music_diffusion_process(
        ...
        nfo("cfg_type: {}, guidance_scale: {}, omega_scale: {}".format(cfg_type, guidance_scale, omega_scale))
        do_classifier_free_guidance = True
        if guidance_scale == 0.0 or guidance_scale == 1.0:
            do_classifier_free_guidance = False
        ...
        if len(oss_steps) > 0:
            ...
            nfo(f"oss_steps: {oss_steps}, num_inference_steps: {num_inference_steps} after remapping to timesteps {timesteps}")
        else:
            timesteps, num_inference_steps = retrieve_timesteps(
                scheduler,
                num_inference_steps=infer_steps,
        ...
        if audio2audio_enable and ref_latents is not None:
            nfo(f"audio2audio_enable: {audio2audio_enable}, ref_latents: {ref_latents.shape}")
        ...
        nfo(f"start_idx: {start_idx}, end_idx: {end_idx}, num_inference_steps: {num_inference_steps}")
        ...
        for i, t in tqdm(enumerate(timesteps), total=num_inference_steps):
            if is_repaint:
                if i < n_min:
                    ...
                    nfo(f"repaint start from {n_min} add {t_i} level of noise")
    ...
    def save_wav_file(self, target_wav, idx, save_path=None, sample_rate=48000, format="wav"):
        if save_path is None:
            ...
            if os.path.isdir(save_path):
                nfo(f"Provided save_path '{save_path}' is a directory. Appending timestamped filename.")
        ...
        if format == "ogg":
            backend = "sox"
        nfo(f"Saving audio to {output_path_wav} using backend {backend}")
        try:
            ...
            sf.write(output_path_wav, target_wav, sample_rate)
    ...
    def load_lora(self, lora_name_or_path, lora_weight):
        if (lora_name_or_path != self.lora_path or lora_weight != self.lora_weight) and lora_name_or_path != "none":
            ...
            self.ace_step_transformer.load_lora_adapter(
                os.path.join(lora_download_path, "pytorch_lora_weights.safetensors"), adapter_name="ace_step_lora", with_alpha=True, prefix=None
            )
            nfo(f"Loading lora weights from: {lora_name_or_path} download path is: {lora_download_path} weight: {lora_weight}")
    ...
    def __call__(
        ...
        nfo(f"Model loaded in {load_model_cost:.2f} seconds.")
        ...
        if audio_duration <= 0:
            ...
            nfo(f"random audio duration: {audio_duration}")
