...
class XFlux(nn.Module):
    ...
    def __init__(self, params: XFluxParams):
        ...
        self.double_blocks = nn.ModuleList(
            [DoubleStreamBlock(self.hidden_size, self.num_heads, mlp_ratio=params.mlp_ratio, qkv_bias=params.qkv_bias) for i in range(1, params.depth + 1)]
        )

        self.single_blocks = nn.ModuleList([SingleStreamBlock(self.hidden_size, self.num_heads, mlp_ratio=params.mlp_ratio) for i in range(1, params.depth_single_blocks + 1)])
    ...
    def forward(
        ...
        for index_block, block in enumerate(self.double_blocks):
            ...
            if return_intermediate:
                intermediate_double.append([img, txt])
        ...
        for index_block, block in enumerate(self.single_blocks):
            ...
            if return_intermediate:
                intermediate_single.append([img_, txt_])
