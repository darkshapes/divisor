...
from divisor.mmada.modeling_mmada import MMadaModelLM
from divisor.mmada.sampling import get_mask_schedule

from divisor.mmada.prompting_utils import UniversalPrompting
...
MODEL_CHOICES = ["MMaDA-8B-Base", "MMaDA-8B-MixCoT", "TraDo-4B-Instruct", "TraDo-8B-Instruct"]
MODEL_ACTUAL_PATHS = {
    "MMaDA-8B-Base": DEFAULT_MODEL_PATH,
    "MMaDA-8B-MixCoT": "Gen-Verse/MMaDA-8B-MixCoT",
    "TraDo-4B-Instruct": "Gen-Verse/TraDo-4B-Instruct",
    "TraDo-8B-Instruct": "Gen-Verse/TraDo-8B-Instruct",
}
...
def _load_model_and_tokenizer_core(model_path_to_load, model_display_name_for_status):
    ...
    try:
        ...
        MODEL = MMadaModelLM.from_pretrained(model_path_to_load, trust_remote_code=True, torch_dtype=torch_dtype).to(DEVICE).eval()
...
@torch.no_grad()
def generate_viz_wrapper_lm(prompt_text, steps, gen_length, block_length, temperature, cfg_scale, remasking_strategy, thinking_mode_lm):
    ...
    yield get_highlighted_text_tuples(x, input_ids, prompt_len, TOKENIZER, MASK_ID, raw_prompt_attention_mask), f"Starting generation: Prompt ({prompt_len} tokens) + Initial Masks"
    ...
    if block_length <= 0 or gen_length % block_length != 0:
        yield (
            get_highlighted_text_tuples(x, input_ids, prompt_len, TOKENIZER, MASK_ID, raw_prompt_attention_mask),
...
@torch.no_grad()
def generate_viz_wrapper(uploaded_image_pil, prompt_text, steps, gen_length, block_length, temperature, cfg_scale, remasking_strategy, thinking_mode_mmu):
    global MODEL, TOKENIZER, MASK_ID, DEVICE
...
with gr.Blocks(css=css_styles, theme=theme) as demo:
    ...
    gr.Markdown(
        "MMaDA is a novel class of multimodal diffusion foundation models designed to achieve superior performance across diverse domains such as textual reasoning, multimodal understanding, and text-to-image generation"
    )
    ...
    with gr.Row():
        model_select_radio = gr.Radio(label="Select Text Generation Model", choices=MODEL_CHOICES, value=MODEL_CHOICES[0])
    ...
    with gr.Row():
        with gr.Column(scale=2):
            prompt_input_box_t2i = gr.Textbox(
                label="Enter your prompt:", lines=3, value="A sea turtle swimming near a coral reef in the ocean, with a clear blue sky and water in the background."
            )
    ...
    run_button_ui_t2i.click(
        fn=generate_viz_wrapper_t2i, inputs=[prompt_input_box_t2i, steps_slider_t2i, guidance_scale_slider_t2i, scheduler_radio_t2i], outputs=[output_image_t2i, output_status_t2i]
    )
    ...
    demo.load(fn=initialize_default_model, inputs=None, outputs=[model_select_radio, model_load_status_box], queue=True)

    def clear_outputs():
        return None, None, None  # Clear image, visualization, and final text

    clear_button_ui_lm.click(fn=clear_outputs, inputs=None, outputs=[image_upload_box, output_visualization_box_lm, output_final_text_box_lm], queue=False)
