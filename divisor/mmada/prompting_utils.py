...
class UniversalPrompting:
    def __init__(
        ...
        if not use_reserved_token:
            ...
            if end_header_tokens and len(end_header_tokens) > 0 and end_header_tokens[0]:
                ...
                special_tokens_dict = {"additional_special_tokens": ["<|start_header_id|>", "<|end_header_id|>", "<|eot_id|>"]}
    ...
    def t2i_prompt(self, text_ids, image_ids, labels):
        ...
        for i in range(len(text_ids)):
            ...
            temp_ids = torch.cat([torch.tensor(temp_ids).to(device), self.sptids_dict["<|soi|>"].to(device), image_ids[i], self.sptids_dict["<|eoi|>"].to(device)], dim=0)
