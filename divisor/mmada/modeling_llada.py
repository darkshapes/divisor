...
class LayerNormBase(nn.Module):
    ...
    def _cast_if_autocast_enabled(self, tensor: torch.Tensor, dtype: Optional[torch.dtype] = torch.float32) -> torch.Tensor:
        if torch.is_autocast_enabled():
            return tensor.to(dtype=dtype or torch.get_autocast_dtype(tensor.device.type))
        else:
            return tensor
...
class LayerNorm(LayerNormBase):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.low_precision:
            ...
            context = get_autocast_context(module_device)
            with context:
...
class RMSLayerNorm(LayerNormBase):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        context = get_autocast_context(x.device)
        with context:
            og_dtype = x.dtype
...
class RotaryEmbedding(nn.Module):
    ...
    def get_rotary_embedding(self, seq_len: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        context = get_autocast_context(device)
        with context:
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        context = get_autocast_context(q.device)
        with context:
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        # Attention input projection. Projects x -> (q, k, v)
        bias_value = config.include_bias or config.include_qkv_bias
        head_dim = config.d_model // config.n_heads
        q_proj_out_dim = config.d_model
        k_proj_out_dim = config.effective_n_kv_heads * head_dim
        v_proj_out_dim = config.effective_n_kv_heads * head_dim
        self.q_proj = nn.Linear(config.d_model, q_proj_out_dim, bias=bias_value, device=config.init_device)
        self.k_proj = nn.Linear(config.d_model, k_proj_out_dim, bias=bias_value, device=config.init_device)
        self.v_proj = nn.Linear(config.d_model, v_proj_out_dim, bias=bias_value, device=config.init_device)
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    # shape: (batch_size, seq_len, d_model)
                    x, cache = self._activation_checkpoint_fn(block, x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache)
                else:
                    # shape: (batch_size, seq_len, d_model)
                    x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache)
                if attn_key_values is not None:
