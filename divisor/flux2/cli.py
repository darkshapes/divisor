...
from divisor.flux2.util import FLUX2_MODEL_INFO, load_ae, load_flow_model, load_mistral_small_embedder, device, device_name
...
def coerce_value(key: str, raw: str):
    ...
    if key == "upsample_prompt_mode":
        ...
        if v in UPSAMPLING_MODE_FIELDS:
            return v
        raise ValueError(f"invalid upsample_prompt_mode: {v}. Must be one of: {', '.join(UPSAMPLING_MODE_FIELDS)}")
...
def main(
    ...
    **overwrite,
    ...
    model = load_flow_model(model_name, debug_mode=debug_mode, device="cpu" if cpu_offloading else device_name)
        ...
        if not single_eval:
            ...
            if not line:
                ...
                if "prompt" in updates and mistral.test_txt(updates["prompt"]):
                    print("Your prompt has been flagged for potential copyright or public personas concerns. Please choose another.")
                    updates.pop("prompt")
            ...
            with torch.no_grad():
                ...
                if cfg.upsample_prompt_mode == "openrouter":
                    try:
                        ...
                        if not api_key:
                            try:
                                entered = input("OPENROUTER_API_KEY not set. Enter it now (leave blank to skip OpenRouter upsampling): ").strip()
                            except (EOFError, KeyboardInterrupt):
                                entered = ""
                            if entered:
                        ...
                        if cfg.upsample_prompt_mode == "openrouter":
                            ...
                            try:
                                sampling_params_input = input("Enter OpenRouter sampling params as JSON or key=value (blank to use defaults): ").strip()
                            except (EOFError, KeyboardInterrupt):
                                sampling_params_input = ""
                            ...
                            if sampling_params_input:
                                ...
                                if not parsed_ok:
                                    ...
                                    tokens = [tok for tok in sampling_params_input.replace(",", " ").split(" ") if tok]
                                    for tok in tokens:
                                        if "=" not in tok:
                                            continue
                                        k, v = tok.split("=", 1)
                                ...
                                if default_params:
                                    ...
                                    print(f"  Using default OpenRouter sampling params for {model_key}: {sampling_params}")
                                else:
                                    print(f"  Setting no OpenRouter sampling params: not set for this model ({model_key})")
                            ...
                            upsampled_prompts = openrouter_api_client.upsample_prompt([cfg.prompt], img=[img_ctx] if img_ctx else None)  # type: ignore
                    ...
                    upsampled_prompts = mistral.upsample_prompt([cfg.prompt], img=[img_ctx] if img_ctx else None)  # type: ignore
                    prompt = upsampled_prompts[0] if upsampled_prompts else cfg.prompt
                else:
                ...
                ctx = mistral([prompt]).to(precision)
                ...
                randn = torch.randn(shape, generator=generator, dtype=precision, device=device)
