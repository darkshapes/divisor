...
import math
from typing import Optional, List, Callable, Iterable, Union
...
def _layer_norm_fwd_1pass_kernel(
    ...
    x = tl.load(X + cols, mask=cols < N, other=0.0).to(torch.float32)
    if HAS_ROWSCALE:
        rowscale = tl.load(ROWSCALE + row).to(torch.float32)
        x *= rowscale
    if HAS_DROPOUT:
    ...
    if HAS_X1:
        x1 = tl.load(X1 + cols, mask=cols < N, other=0.0).to(torch.float32)
        if HAS_ROWSCALE:
            rowscale = tl.load(ROWSCALE + M + row).to(torch.float32)
            x1 *= rowscale
        if HAS_DROPOUT:
            # Compute dropout mask
            # 7 rounds is good enough, and reduces register pressure
        ...
        x += x1
    if HAS_RESIDUAL:
        residual = tl.load(RESIDUAL + cols, mask=cols < N, other=0.0).to(torch.float32)
        x += residual
    if STORE_RESIDUAL_OUT:
    ...
    mask = cols < N
    w = tl.load(W + cols, mask=mask).to(torch.float32)
    if zero_centered_weight:
        w += 1.0
    if HAS_BIAS:
        b = tl.load(B + cols, mask=mask).to(torch.float32)
    x_hat = (x - mean) * rstd if not IS_RMS_NORM else x * rstd
    y = x_hat * w + b if HAS_BIAS else x_hat * w
    # Write output
    tl.store(Y + cols, y, mask=mask)
    if HAS_W1:
        w1 = tl.load(W1 + cols, mask=mask).to(torch.float32)
        if zero_centered_weight:
            w1 += 1.0
        if HAS_B1:
            b1 = tl.load(B1 + cols, mask=mask).to(torch.float32)
...
def _layer_norm_bwd_kernel(
    ...
    if RECOMPUTE_OUTPUT and HAS_BIAS:
        b = tl.load(B + cols, mask=mask, other=0.0).to(torch.float32)
    if HAS_DY1:
        w1 = tl.load(W1 + cols, mask=mask).to(torch.float32)
        if zero_centered_weight:
            w1 += 1.0
    dw = tl.zeros((BLOCK_N,), dtype=torch.float32)
    if HAS_BIAS:
        db = tl.zeros((BLOCK_N,), dtype=torch.float32)
    if HAS_DY1:
        dw1 = tl.zeros((BLOCK_N,), dtype=torch.float32)
        if HAS_B1:
            db1 = tl.zeros((BLOCK_N,), dtype=torch.float32)
    row_end = min((row_block_id + 1) * rows_per_program, M)
    for row in range(row_start, row_end):
        # Load data to SRAM
        x = tl.load(X + cols, mask=mask, other=0).to(torch.float32)
        dy = tl.load(DY + cols, mask=mask, other=0).to(torch.float32)
        if HAS_DY1:
            dy1 = tl.load(DY1 + cols, mask=mask, other=0).to(torch.float32)
        if not IS_RMS_NORM:
        ...
        if not IS_RMS_NORM:
            ...
            dx = (wdy - xhat * c1) * rstd
        if HAS_DRESIDUAL:
            dres = tl.load(DRESIDUAL + cols, mask=mask, other=0).to(torch.float32)
            dx += dres
        # Write dx
        if STORE_DRESIDUAL:
...
class RMSNorm(torch.nn.Module):
    def __init__(self, hidden_size, eps=1e-5, dropout_p=0.0, zero_centered_weight=False, device=None, dtype=None):
